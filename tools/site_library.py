...
<div class="grid3">
  ...
  <div class="feat">
    <h3>Nuthatch</h3>
    <p>An open single-seat ultralight aircraft design. The reason Part 103 is a
    first-class citizen here rather than an afterthought: an aircraft that needs no
    approved flight manual has no source document to transcribe, so its checklist has
    to be authored — and the format has to say so honestly.</p>
    <p><a href="https://github.com/allenmcghan/nuthatch" rel="noopener">Repository</a></p>
  </div>
  <div class="feat">
...
<p class="lede">A profile describes how one airframe is wired — limits, sensor maps,
calibration baselines, unit preferences — so Junco supports a new aircraft without a
firmware change. Profiles are TOML rather than JSON, and that difference is
deliberate.</p>

<p>A checklist is a document with a source you can cite and a verification state that
has to travel with it. A profile is hand-edited hardware configuration: there is no
upstream document to transcribe and no provenance to record, and someone will edit it
in a text editor at a hangar with cold hands. TOML is the better shape for that, so
the two formats are not forced to become one.</p>

<table><tbody>
<tr><td><b>ParaPlane PM-2</b><br><span class="muted">Twin-engine two-stroke powered
    parachute</span></td>
    <td><a href="https://github.com/allenmcghan/openchecklists/blob/main/profiles/pm2/pm2.toml"
    rel="noopener">pm2.toml</a> · <span class="muted">draft</span></td></tr>
</tbody></table>

<p class="hint">The format specification is
<a href="https://github.com/allenmcghan/openchecklists/blob/main/spec/aircraft-profile.md"
rel="noopener">spec/aircraft-profile.md</a>. Junco's own copy redirects to it, so there
is one authoritative definition rather than two that drift apart.</p>
